...
RazorAHRS::RazorAHRS(const std::string &port, DataCallbackFunc data_func, ErrorCallbackFunc error_func, Mode mode, std::string sensorname, char *participantname,
...
RazorAHRS::_makepathfile(sensorname, participantname);
...
RazorAHRS::~RazorAHRS()
{
  ...
  close(_serial_port);
	...
	//RazorAHRS::_datafile.close();
	RazorAHRS::_write2file.close();
...
void
RazorAHRS::_makepathfile(std::string _sensorname, char *_participantname)
{
 
  
  string str = _participantname; // convert char * to string
  
string directoryName = "data";
string fullpath = (RazorAHRS::_get_current_path()+"/"+directoryName+"/"+str+"/");
mkdir( ((fullpath).c_str()),S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);


string pathfileName = (fullpath+_sensorname+RazorAHRS::_get_current_time()+".csv");
RazorAHRS::_write2file.open( pathfileName.c_str(), ios::out );
// RazorAHRS::_write2file << "Sample, Yaw, Pitch, Roll" << endl;
...
string
RazorAHRS::_get_current_path()
{
char *real_path = realpath("../../../", NULL); // ./ for current path
...
RazorAHRS::_get_current_time()
{
  ...
  strftime (buffer,sizeof(buffer),"_%d-%m-%Y_%H-%M-%S",timeinfo);//generate string SA_TEST_DATE_TIME
  
  string str(buffer); // <http://stackoverflow.com/questions/8960087/how-to-convert-a-char-array-to-a-string>
    
    
  return str;
  // <http://stackoverflow.com/questions/25030055/add-date-and-time-to-a-file-name-in-c>
}
...
int
RazorAHRS::_modulus(int a, int b)
{
 if (b < 0)
	return _modulus(-a,-b);
 int ret = a % b;
...
//            cout << _input_buf.ypr[0] << "," << _input_buf.ypr[1]  << "," << _input_buf.ypr[2] << endl;
 	  ...
 	  RazorAHRS::_write2file  << _sample  << ","  << _input_buf.ypr[0] << "," << _input_buf.ypr[1]  << "," << _input_buf.ypr[2] << endl;
          _sample++;
	  ...
	  RazorAHRS::_write2file  << _sample  << ","  << _input_buf.amg[0] << "," << _input_buf.amg[1]  << "," << _input_buf.amg[2]
					      << ","  << _input_buf.amg[3] << "," << _input_buf.amg[4]  << "," << _input_buf.amg[5] 
					      << ","  << _input_buf.amg[6] << "," << _input_buf.amg[7]  << "," << _input_buf.amg[8] << endl;
          _sample++;
