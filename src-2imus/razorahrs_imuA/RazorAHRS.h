...
#include <string>	// .c_str()
					// get c string equilavent which returns a pointer
					// to an array that contains a null-terminated sequence of characters
...
class RazorAHRS
{
  public:
    ...
    RazorAHRS(const std::string &port, DataCallbackFunc data_func, ErrorCallbackFunc error_func, Mode mode, std::string sensorname, char *participantname,
  ...
  private:
	...
	void _makepathfile(std::string _sensorname, char *_participantname);
	
	
	string _get_current_path();
...
// void _push_back_vectors(float X, float Y, float Z);
